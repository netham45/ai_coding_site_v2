...
- Prompts: make the parent-node prompt contract explicit enough for the AI to generate the hierarchy needed for the sample `cat` program, register the generated child plan/layout by filename, and then let the leaf task implement it.
...
4. Then add a scoped parent decomposition workflow for this runtime narrative without silently changing the global default ladders, including an explicit register-by-filename handoff for generated child plans/layouts.
...
- Phase 4 real target: `timeout 1020 python3 -m pytest -q tests/e2e/test_e2e_automated_full_tree_cat_runtime_real.py`
...
- Parent-generated child plans/layouts are not treated as authoritative until they are explicitly registered through the CLI by filename.
