...
- Open:
  - `tests/e2e/test_e2e_incremental_parent_merge_real.py`, `tests/e2e/test_e2e_full_epic_tree_runtime_real.py`, `tests/e2e/test_e2e_rebuild_cutover_coordination_real.py`, and `tests/e2e/test_tmux_codex_idle_nudge_real.py` are quarantined from canonical passing real-E2E command sets under the stricter live-run-equivalence doctrine because they still contain synthetic workflow progression, lifecycle forcing, or other non-live-run-equivalent steps in at least part of their narrative
  - `tests/e2e/test_e2e_incremental_parent_merge_real.py` still uses `/api/subtasks/complete` and related operator-driven progression helpers as part of its main runtime narrative, so the broader `tests/e2e/` family is not yet uniformly “runtime advances itself” proof
