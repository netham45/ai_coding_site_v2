...
pytestmark = [pytest.mark.e2e_bringup, pytest.mark.requires_ai_provider]
...
def test_e2e_rebuild_cutover_coordination_blocks_upstream_rectify_while_authoritative_run_is_active(
    ...
    try:
        ...
        bind_result = harness.cli("session", "bind", "--node", child_id)
        ...
        assert any(item["blocker_type"] == "active_primary_sessions" for item in coordination_payload["blockers"])
