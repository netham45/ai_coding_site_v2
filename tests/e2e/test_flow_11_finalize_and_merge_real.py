...
def test_flow_11_finalize_and_merge_runs_against_real_daemon_real_cli_and_real_git(real_daemon_harness, tmp_path) -> None:
    ...
    assert parent_run_show_payload is not None, (
        "Expected the finalize/merge parent flow to produce durable run state after binding a real tmux/provider session.\n"
    ...
    child_run_show_payload = None
    child_last_pane_text = ""
    child_deadline = time.time() + 60.0
    while time.time() < child_deadline:
        run_show = real_daemon_harness.cli("node", "run", "show", "--node", child_id)
        assert run_show.exit_code == 0, run_show.stderr
        child_run_show_payload = run_show.json()
        child_last_pane_text = subprocess.run(
            ["tmux", "capture-pane", "-p", "-t", child_session_name],
            text=True,
            capture_output=True,
            check=False,
        ).stdout
        if (
            child_run_show_payload["latest_attempt"] is not None
            or child_run_show_payload["state"]["last_completed_compiled_subtask_id"] is not None
            or child_run_show_payload["run"]["run_status"] in {"FAILED", "PAUSED", "COMPLETED", "COMPLETE"}
        ):
            break
        time.sleep(2.0)

    assert child_run_show_payload is not None, (
        "Expected the finalize/merge child flow to produce durable run state after binding a real tmux/provider session.\n"
        f"session_name={child_session_name}\n"
        f"pane_text=\n{child_last_pane_text}"
    )
