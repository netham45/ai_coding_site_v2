...
| GAP-005 | F15, F19, F35 | Child materialization and scheduling | likely_new_pseudocode | critical | in_review | Layout and child ideas exist, and the automated full-tree `cat` runtime preimplementation note now freezes the requested narrative boundary, but generated-layout authority, daemon child auto-start, and scoped parent decomposition still need implementation-grade fold-in. | Use the new preimplementation note to drive phased runtime work for generated-layout materialization, child auto-start, and scoped decomposition, then verify required DB state. |
...
| GAP-007 | F23 | Testing persistence model | likely_new_db_structure | high | implemented | F23 now persists `test_results`, mirrors latest per-attempt state in `subtask_attempts.testing_json`, and exposes daemon/CLI testing summaries plus retry-aware routing. | Revisit only if later phases need richer per-command/per-file result normalization. |
