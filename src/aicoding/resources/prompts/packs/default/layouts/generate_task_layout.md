...
- if the current request already names the concrete file or module to change and the exact validation command to pass, create exactly one implementation task by default
- do not split concrete implementation work into separate diagnosis, discovery, reproduction, or verification-only tasks unless the current request is ambiguous or explicitly requires those as separate tasks
- do not preserve ancestor dependency shapes once the current plan request is already concrete enough to execute directly
